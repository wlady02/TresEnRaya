...
public class Board extends javax.swing.JFrame {   
	...
	private void onClick(ActionEvent event) {
		...
		if(movement == Movement.NOT_ALLOWED){
			...
			String[] cellValues = game.getBoard(celda); //adaptar getBoard al nuevo main
			for(int i = 0; i < Game.NUMBER_OF_CELLLS; i++)
	...
	public void getBoard(int resp) {
		Movement movement = game.movePlayer2(resp);
		if(movement == Movement.NOT_ALLOWED){
			label.setText("Movimiento no válido.");
		}else{
			String[] cellValues = game.getBoard(resp); //adaptar getBoard al nuevo main
			for(int i = 0; i < Game.NUMBER_OF_CELLLS; i++)
				cells[i].setText(cellValues[i]);
	...
	public int getMove() { //celda para setMove Game-proxy
	return celda;
	}
}